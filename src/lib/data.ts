...
// Simplified type for dummy workouts
...
export const dummyWorkouts: DummyWorkout[] = generateDummyWorkouts();

function generateDummyWorkouts(): DummyWorkout[] {
  const workouts: DummyWorkout[] = [];
    ...
    for (let j = 0; j < workoutsPerDay; j++) {
      ...
      for (let k = 0; k < numTypes; k++) {
        ...
        do {
          type =
            defaultWorkoutTypes[
              Math.floor(Math.random() * defaultWorkoutTypes.length)
            ];
